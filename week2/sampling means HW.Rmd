...
for (iter in 1:100) {
  ...
  # There is a really nice function in R called "sample" which lets us take a random sub sample of our data say I'd like to take a sample of 25 observations 
  obs = 25
  total_samps = 100
  ...
  colnames(sim_power) <- c("Number_Obs","Mean_value_at_5percentile")
  #Reset sample_means
  sample_means <- NULL
  sample_means = rep(0,total_samps)
  ...
  sim_power <-
    filter(sim_power, Mean_value_at_5percentile > 9) %>%  
    filter(`Mean_value_at_5percentile` == min(`Mean_value_at_5percentile`))
  
  ans[iter] <- sim_power[1,1]  #assign the min sample-size that yields the desirable value to one of the entry in the 'ans' vector
}
